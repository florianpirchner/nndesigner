...
public class MoveInOutPutsInCommonTrigger implements ModelChangeTrigger {
	...
	public Option<Command> localChangesAboutToCommit(Collection<Notification> notifications) {
		final Collection<org.eclipse.gmf.runtime.notation.Node> toMakeAutosize = Sets.newLinkedHashSet();
		for (Notification notif : notifications) {
			org.eclipse.gmf.runtime.notation.Node nd = (org.eclipse.gmf.runtime.notation.Node) notif.getNewValue();
		...
		if (toMakeAutosize.size() > 0) {
			Command result = new RecordingCommand(domain) {
				...
				protected void doExecute() {
					for (org.eclipse.gmf.runtime.notation.Node node : toMakeAutosize) {
